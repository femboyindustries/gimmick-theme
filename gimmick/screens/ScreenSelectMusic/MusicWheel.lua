-- The MusicWheel rendering works by what I call "drawfunction injection": when
-- the game calls OnCommands in order to initialize the music wheel items, we
-- instead set a drawfunction, overriding whatever rendering it had, and using
-- actors defined earlier in the screen's scope to manually re-render the items.
--
-- This means we don't have to deal with rendering a wheel from scratch that
-- follows the real one by reading the data or anything silly like that; we can
-- just read back the text from the actors in the item, and re-render it in
-- a way that suits us.
--
-- This unfortunately doesn't contain any chart data, which is why we fetch
-- every loaded chart from the game and search through it for a match with the
-- title / artist / subtitle fields given to us by the underlying wheel item
-- actor. This is cached, so it's not that big of a deal. (clueless)
--
-- We can detect which actor is the selected item by seeing the order in which
-- the items are drawn; the one drawn last will always be the selected one.
-- After this, to ease individual actors, we index them with which order they
-- were defined in.
...
function MusicWheel.init(ctx)
  ...
  MusicWheel.itemDrawFunc = function(index)
